 ...
 */
export async function getUserSalt(jwt: string, decodedJwt: DecodedJwt): Promise<string> {
 ...
 */
export async function getZkLoginProof(jwt: string, userSalt: string): Promise<any> {
  try {
    ...
    const proofPromise = (async () => {
      const keypair = getSession<{ publicKey: string }>(ZKLOGIN_CONFIG.storageKeys.ephemeralKeyPair);
      const response = await fetch(ZKLOGIN_CONFIG.prover.url, {
        ...
        body: JSON.stringify({
          jwt,
          extendedEphemeralPublicKey: keypair?.publicKey || '',
          maxEpoch: getSession<number>(ZKLOGIN_CONFIG.storageKeys.maxEpoch) || 0,
          jwtRandomness: getSession<string>(ZKLOGIN_CONFIG.storageKeys.randomness) || '',
          salt: userSalt,
          keyClaimName: 'sub',
        }),
      });
    ...
    if (error instanceof Error && error.message.includes('Too many proof requests')) {
      throw error; // Re-throw with specific message
    }
    
    throw new Error('Failed to generate zero-knowledge proof. Please try again.');
  }
}
...
export async function signAndExecuteZkLoginTransaction(
  ...
  try {
    ...
    // Get stored data
    const ephemeralKeyPairData = getEphemeralKeyPair();
    ...
    if (!ephemeralKeyPairData || !zkProof || !maxEpoch || !jwt || !userSalt) {
      throw new Error('Missing zkLogin session data');
    }
    
    // Reconstruct ephemeral keypair from stored private key
    const privateKeyBytes = Buffer.from(ephemeralKeyPairData.privateKey, 'base64');
